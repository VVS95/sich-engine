...
impl AssetLoader for GscBmpLoader {
    ...
    ) -> impl bevy::utils::ConditionalSendFuture<Output = Result<Self::Asset, Self::Error>> + 'a {
        Box::pin(async move {
            let mut bytes = Vec::new();
            reader.read_to_end(&mut bytes).await?;

            // Хардкодимо шлях до палітри. Вона має лежати у папці assets/
            ...
            for y in 0..height {
                ...
                for x in 0..width {
                    if row_start + x < bytes.len() {
                        ...
                        // Прозорість для чорного кольору (індекс 0)
